...
### 为什么优先 DAG 而非 T2I 预生成 Reference？

一个自然的问题是：既然 reference 质量如此重要，为什么不直接用 **T2I（Text-to-Image）先生成理想的 reference 图**，然后全部用 **S2V（Subject-to-Video）** 生成视频？

**方案对比**：

| 方案 | Reference 来源 | 视频生成 | 一致性 | 视频质量 |
|------|---------------|---------|--------|---------|
| **DAG 调度（当前）** | 从生成视频中 grounding | T2V + S2V | 依赖 grounding | T2V 质量高 |
| **T2I + S2V** | T2I 预生成 | 全部 S2V | 天然高一致性 | S2V 可能较弱 |

**我们选择 DAG 优先的原因**：

1. **T2V 质量通常优于 S2V**
   - T2V 模型（如 Wan2.1-T2V-14B）生成的视频动作更自然、更流畅
   - S2V 有时会"过拟合"到 reference 图，导致动作僵硬、姿态单一
   - 首镜头使用 T2V 可以获得最高质量的基础视频

2. **T2I 生成的 reference 可能与场景不协调**
   - T2I 生成的是"理想化"的参考图（如正面、均匀光线）
   - 但实际视频场景可能有特定的光线、角度、风格
   - 例如：T2I 生成的正面人脸用于逆光场景会显得不自然

3. **保持系统的"自举"特性**
   - 当前系统完全自举：从自己生成的视频中提取 reference
   - 这体现了 agentic 的闭环特性：perceive → reason → act
   - 引入 T2I 预生成会打破这个优雅的闭环

4. **DAG 调度本身就是一个方法论创新**
   - "叙事顺序 ≠ 最优生成顺序" 是一个可验证的核心洞察
   - 这个 contribution 不依赖于具体的模型，具有通用性

### T2I Fallback 策略（v2.6 新增）

对于极端困难的 case（如实体在所有镜头都是背影/遮挡），DAG 调度也无法获得高质量 reference。此时需要 **T2I 作为兜底方案**。

**核心策略：LLM 预筛 + 动态 Retry 预算**

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    T2I Fallback Agent 决策流程                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  阶段 1：执行前 LLM 预判（基于 DAG 质量预测）                                 │
│  ───────────────────────────────────────────                                │
│    ┌──────────────────┬────────────────┬────────────┐                       │
│    │ 预测质量 Q_best  │ 策略           │ Retry 预算 │                       │
│    ├──────────────────┼────────────────┼────────────┤                       │
│    │ < 0.3 (很差)     │ 直接 T2I       │ 0          │                       │
│    │ 0.3 - 0.5 (边缘) │ 尝试 DAG       │ 1 次       │                       │
│    │ >= 0.5 (正常)    │ 正常 DAG 流程  │ 3 次       │                       │
│    └──────────────────┴────────────────┴────────────┘                       │
│                                                                             │
│  阶段 2：执行中验证                                                          │
│  ─────────────────────                                                      │
│    - 每次生成后检查 grounding 质量                                           │
│    - 若 retry 用尽仍失败 → Fallback 到 T2I                                   │
│                                                                             │
│  阶段 3：详细日志记录                                                        │
│  ─────────────────────                                                      │
│    - 记录 LLM 预判结果                                                       │
│    - 记录每次尝试的 grounding 质量                                           │
│    - 记录 fallback 原因                                                     │
│    - 导出到 t2i_fallback_log.json                                           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

**为什么用"LLM 预筛 + 动态预算"而非纯规则或纯失败后 fallback？**

| 方案 | 优点 | 缺点 |
|------|------|------|
| 纯 LLM 预判 | 节省时间 | 可能误判，放弃本可成功的 case |
| 纯失败后 fallback | 不会误判 | 浪费计算资源，用户体验差 |
| **LLM 预筛 + 动态预算（本方案）** | 兼顾效率与准确性 | — |

**使用方式**：

```bash
# 启用 T2I Fallback（默认开启）
bash scripts/run_multi_seed.sh 3

# 自定义阈值
python phase1_poc/run_demo.py \
    --enable_t2i_fallback \
    --t2i_quality_threshold_immediate 0.3 \
    --t2i_quality_threshold_normal 0.5 \
    --t2i_low_budget_retries 1
```
...
```
======================================================================
[T2IFallbackAgent] 预判决策 | char_mystery
======================================================================
  实体类型: character
  预测质量: 0.25
  🔄 策略: t2i_immediate
  Retry 预算: 0
  理由: 预测质量 0.25 < 0.3，所有镜头都难以获得高质量 reference
======================================================================

[T2IFallbackAgent] 🔄 char_mystery | 尝试 1/1 | 动作: t2i_fallback

======================================================================
[T2IFallbackAgent] 🔄✅ 最终结果 | char_mystery
======================================================================
  结果: t2i_success
  尝试次数: 1
  总耗时: 2.3s
  Reference: output/t2i_refs/char_mystery.png
======================================================================
```
...
| 文件 | 说明 |
|------|------|
| `agents/t2i_fallback_agent.py` | T2I Fallback Agent 核心实现 |
| `output/t2i_fallback_log.json` | Fallback 决策日志（每次运行生成） |

这种"DAG 优先 + T2I 兜底"的策略，既保持了 T2V 的质量优势和系统的自举特性，又为极端 case 提供了后备方案。整个决策过程由 LLM 驱动，体现了 Agentic 特性。
