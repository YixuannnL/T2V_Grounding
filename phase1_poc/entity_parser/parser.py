...
4. 评估每个实体的 grounding 优先级（high=人物/核心物体，medium=场景，low=背景）
...
class EntityParser:
    ...
    def set_global_caption(self, global_caption: str):
        """
        设置全局背景描述（整个视频的叙事背景）。
        会在解析 Shot 1 之前先从 global_caption 提取所有实体，
        此后每个 shot 解析时都会把 global_caption 作为背景注入 prompt，
        帮助 LLM 做跨镜头的共指消解。
        """
        self._global_caption = global_caption.strip()
        # 用 shot_id=0 解析全局实体，建立实体图谱
